    ...
    sample_documents = [
        {
            ...
            "summary_embedding": np.repeat(0.1, 768).astype(np.float32).tolist(),
            "keywords_embedding": np.repeat(0.1, 768).astype(np.float32).tolist()
        },
        {
            ...
            "summary_embedding": np.repeat(0.2, 768).astype(np.float32).tolist(),
            "keywords_embedding": np.repeat(0.2, 768).astype(np.float32).tolist()
        },
        {
            ...
            "summary_embedding": np.repeat(0.3, 768).astype(np.float32).tolist(),
            "keywords_embedding": np.repeat(0.3, 768).astype(np.float32).tolist()
        },
        {
            ...
            "summary_embedding": np.repeat(0.4, 768).astype(np.float32).tolist(),
            "keywords_embedding": np.repeat(0.4, 768).astype(np.float32).tolist()
        },
        {
            ...
            "summary_embedding": np.repeat(0.5, 768).astype(np.float32).tolist(),
...
def hybrid_search(weight_summary_vector, query_summary_vector,
                  ...
                  weight_keywords_text, query_keywords_text, top_k=5):
    """
    执行混合检索 (向量 + BM25)
